...
if args.optimizer1 == "Adagrad":
    ...
    optimizer1 = optim.Adam(
        Autoencoder.parameters(), lr=args.lr1, weight_decay=args.wd1
    )
    ...
    optimizer1 = optim.SGD(
        Autoencoder.parameters(), lr=args.lr1, momentum=0.95, weight_decay=args.wd1
    )
...
def evaluate(data_loader, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            prediction = Autoencoder.Decode(
                batch_latent_recon
            )  # [batch_size, n1_items + n2_items + n3_items]

            prediction[
                his_data.nonzero()
            ] = -np.inf  # mask ui pairs in train & validation set
...
if args.n_cate > 1:
    ...
    mask_train = sp.csr_matrix(
        (np.ones_like(mask_idx_train[0]), (mask_idx_train[0], mapped_mask_iid_train)),
        dtype="float64",
        shape=(n_user, n_item),
    )
    ...
    mask_val = sp.csr_matrix(
        (np.ones_like(mask_idx_val[0]), (mask_idx_val[0], mapped_mask_iid_val)),
        dtype="float64",
        shape=(n_user, n_item),
    )
    ...
    print(
        "Preparing mask for validation & test costs "
...
for epoch in range(1, args.epochs + 1):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if args.anneal_steps > 0:
            lamda = max(
                (1.0 - update_count / args.anneal_steps) * args.lamda, args.anneal_cap
            )
        else:
        ...
        if args.vae_anneal_steps > 0:
            anneal = min(
                args.vae_anneal_cap, 1.0 * update_count_vae / args.vae_anneal_steps
            )
        else:
            anneal = args.vae_anneal_cap

        vae_loss = (
            compute_loss(batch_recon, batch_cate) + anneal * vae_kl
    ...
    if epoch % 5 == 0:
        ...
        if valid_results[1][1] > best_recall:  # recall@20 as selection
            ...
            torch.save(
                model,
                "{}{}_{}lr1_{}lr2_{}wd1_{}wd2_bs{}_cate{}_in{}_out{}_lam{}_dims{}_emb{}_{}_steps{}_scale{}_min{}_max{}_sample{}_reweight{}_wmin{}_wmax{}_{}.pth".format(
                    save_path,
                    args.dataset,
                    args.lr1,
                    args.lr2,
            ...
            torch.save(
                Autoencoder,
                "{}{}_{}lr1_{}lr2_{}wd1_{}wd2_bs{}_cate{}_in{}_out{}_lam{}_dims{}_emb{}_{}_steps{}_scale{}_min{}_max{}_sample{}_reweight{}_wmin{}_wmax{}_{}_AE.pth".format(
                    save_path,
                    args.dataset,
                    args.lr1,
                    args.lr2,
